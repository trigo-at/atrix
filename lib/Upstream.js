...
class Upstream {
	...
	buildOptions(options, overrideOptions) {
		...
		if (this.config.security && this.config.security.strategies && this.config.security.strategies.basic) {
			const { username, secret } = this.config.security.strategies.basic;
			fetchOptions.headers = fetchOptions.headers || {};
			fetchOptions.headers.Authorization = `Basic ${btoa(`${username}:${secret}`)}`;
		}
