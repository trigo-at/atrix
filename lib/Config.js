...
class Config {
	constructor(serviceName, rawConfig, environment) {
		...
		if (process.env[`ATRIX_${serviceName.toUpperCase()}_LOGGER_LEVEL`] && loggerCfg.logger)	 {
			loggerCfg.logger.level = process.env[`ATRIX_${serviceName.toUpperCase()}_LOGGER_LEVEL`];
		}
