...
describe('Config', () => {
    ...
    it('returns full serviceConfig()', () => {
        ...
        expect(sc[0]).to.eql({
            key: 'ATRIX_TEST_DB_CONNECTIONSTRING',
            defaultValue: '<UNCONFIGURED>',
            value: 'conCfg',
            path: ['db', 'connectionString'],
        });
        expect(sc[1]).to.eql({
            key: 'ATRIX_TEST_SWAGGER_SERVICEDEFINITION',
            defaultValue: '<UNCONFIGURED>',
            value: 'sdCfg',
            path: ['swagger', 'serviceDefinition'],
        });
        expect(sc[2]).to.eql({
            key: 'ATRIX_TEST_ENDPOINTS_HTTP_CORS_ORIGIN_0',
            defaultValue: 'o1',
            path: ['endpoints', 'http', 'cors', 'origin', 0],
        });
        expect(sc[3]).to.eql({
            key: 'ATRIX_TEST_ENDPOINTS_HTTP_CORS_ORIGIN_1',
            defaultValue: 'o2',
            path: ['endpoints', 'http', 'cors', 'origin', 1],
        });
        expect(sc[4]).to.eql({
            key: 'ATRIX_TEST_LOGGER_LEVEL',
            defaultValue: 'error',
            value: 'info',
            path: ['logger', 'level'],
        });
