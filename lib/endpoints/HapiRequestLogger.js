...
function parseRequest(request, opts) {
	...
	if (opts.logFullRequest && request.headers && request.headers['content-type']) {
		...
		} else if (request.headers['content-type'].match(/multipart\/form\-data/)) { //eslint-disable-line
			reqData.json = {};
...
function register(server, options, next) {
	const opts = Object.assign({ logFullResponse: false, logFullRequest: false }, options);
