...
function resolveHandlers(baseDir, service) {
	const fileList = walk(baseDir).filter(
		file =>
	...
	const routeExists = (route, method) =>
		R.find(
			R.allPass([R.propEq('path', route), R.propEq('method', method)]),
			ret
		);
	fileList.forEach(file => {
		...
		let routePath = file
			.replace(baseDir, '')
			.replace(/\^/g, '/')
			.replace(/GET|POST|HEAD|DELETE|PUT|OPTIONS|PATCH/, '')
			.replace(/_\.js|\.js/, '');
		if (routePath !== '/' && routePath[routePath.length - 1] === '/') {
			routePath = routePath.substr(0, routePath.length - 1);
		}

		const method = file.match(
			/.*(GET|POST|HEAD|DELETE|PUT|OPTIONS|PATCH)|.js/
		)[1];
		const existing = routeExists(routePath, method);
		if (existing) {
			const msg = `Route conflict: ${existing.method} ${
				existing.path
			} defined by: ${file} conflicts with: ${existing.file}`;
			throw new Error(msg);
		}
