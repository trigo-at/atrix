...
function resolveHandlers(baseDir, service) {
	...
	const fileList = walk(baseDir)
		.filter(file => file.indexOf('index.js') === -1 &&
			file.match(isValidHandlerRegex));
	const resolvedRoutes = [];

	const routeExists = (route, method) => R.find(R.allPass([R.propEq('path', route), R.propEq('method', method)]), resolvedRoutes);
	const routePathReplaceRegex = new RegExp(`GET\\.js$|POST\\.js$|HEAD\\.js$|DELETE\\.js$|PUT\\.js$|OPTIONS\\.js$|PATCH\\.js$|${regexWildcard}\\.js$`);
	...
	fileList.forEach((file) => {
		...
		let routePath = file.replace(baseDir, '').replace(/\^/g, '/')
			.replace(routePathReplaceRegex, '').replace(/_$/, '');
		...
		if (module === symbols.DISABLED || module.handler === symbols.DISABLED || module.proxy === symbols.DISABLED) return;
	...
	service.log.debug(`Resolved handler list from ${baseDir}`, JSON.stringify(resolvedRoutes, null, 2));
