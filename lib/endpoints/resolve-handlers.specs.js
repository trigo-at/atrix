...
describe('resolve-handlers', () => {
	let routes;
	let	differentWildcardRoutes;

	beforeEach(() => {
		routes = resolveHandlers(path.join(__dirname, '../../tests/test-handler-dir'), undefined, service);
		differentWildcardRoutes = resolveHandlers(path.join(__dirname, '../../tests/test-handler-dir-wildcard'), '%', service);
	});

	it('returns object with handler definitions', () => {
	...
	it('resolves paths', () => {
		...
		expect(getPath('/has/multiple/carets', routes)).to.exist;
	});
	...
	it("accepts default wildcard '$' for method and interprets it as all allowed methods", () => {
		expect(getPath('/ends-with', routes).method).to.eql([
			'DELETE',
			'GET',
			'OPTIONS',
			'PATCH',
	...
	it("accepts different wildcard '%' for method and interprets it as all allowed methods", () => {
		expect(getPath('/ends-with', differentWildcardRoutes).method).to.eql([
			'DELETE',
			'GET',
			'OPTIONS',
			'PATCH',
	...
	it('throws Error when duplicate path defined', () => {
		expect(() => resolveHandlers(path.join(__dirname, '../../tests/test-handler-dir-dupl'), undefined, service)).to.throw(Error);
	});

	it('can overwrite path in handler module', () => {
		expect(getPath('/herbert', routes).method).to.eql('GET');
	});

	it('can overwrite HTTP method in handler module', () => {
