...
describe('resolve-handlers', () => {
    ...
    it("accepts default wildcard '%' for method and interprets it as all allowed methods", () => {
        expect(getPath('/ends-with', routes).method).to.eql(['DELETE', 'GET', 'OPTIONS', 'PATCH', 'POST', 'PUT']);
    });
    ...
    it('throws Error when duplicate path defined', () => {
        expect(() =>
            resolveHandlers(path.join(__dirname, '../../tests/test-handler-dir-dupl'), undefined, service)
        ).to.throw(Error);
    });
    ...
    it('can set route config in handler module', () => {
        expect(getPath('/emil', routes).options).to.eql({
            query: false,
        });
    });
});
