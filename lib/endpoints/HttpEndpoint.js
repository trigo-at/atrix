...
// TODO: Add once signe link plugin is updated
...
class HttpEndpoint {
    ...
    async setupAuthStrategies() {
        if (this.isSecured) {
            // TODO: Add once signed link plugin is updated 
            ...
            if (Array.isArray(this.securityConfig.endpoints) && this.securityConfig.strategies.jwt) {
                ...
                if (this.securityConfig.endpoints.signedlink) {
                    ...
                    if (this.isSecured && endpointPattern.some(x => path.match(x))) {
                        if (config.auth) {
                            throw new Error(
                                `CONFIGURATION ERROR: Route: "${path}" cannot be secured using "signedlink" strategy. It is already secured with: "${config.auth.strategy
                                }" strategy.`
                            ); //eslint-disable-line
                        }
