...
const configSchema = Joi.object({
    ...
    validation: Joi.object({
        verbose: Joi.array()
            ...
            .default([])
            .description(
                'list of expressions for endpoints that return full detailes in error response when validation rules fail'
            ),
    })
...
class HttpEndpoint {
    ...
    setupRequestInterface() {
        this.service.request = async (options, parentRequest) => {
            ...
            if (parentRequest && parentRequest.info.id && parentRequest.headers) {
    ...
    createRouteConfig(path, cfg) {
        ...
        if (this.config.validation && this.config.validation.verbose) {
            const endpointPattern = this.config.validation.verbose.map(x => new RegExp(x));
