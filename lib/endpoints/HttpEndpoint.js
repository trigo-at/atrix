...
const SecuritySchema = Joi.object({
    strategies: Joi.object({
        jwt: Joi.object({
            ...
            jwks: Joi.object({
                jwksUri: Joi.string().description('Dynamically fetch the signing key from JWKS URL.'),
                cache: Joi.boolean().default(true).description('cache signing keys to reduce requests'),
                rateLimit: Joi.boolean().default(true).description('whether to rate limit jwks requests'),
                jwksRequestsPerMinute: Joi.number().default(2).description('How often to refresh the keys from the server'),
            }).description('Use JWKS to fetch token signing keys. If configured a configured "secret" will be ignored'),
            verifyOptions: Joi.object({
                audience: Joi.string().description('verify "aud" claim'),
                issuer: Joi.string().description('verify "iss" claim'),
                algorithms: Joi.array().items(Joi.string().valid(
                    'RS256',
                    'RS384',
                    'RS512',
                    'PS256',
                    ...
                    'EdDSA'))
                    .default(['RS256'])
                    .description('the algorithm passed to the jsonwebtoken.verify(...) method./ If set top level "algorithm" setting is ignored')
            }).description('Options passed to the jsonwebtoken.verify(...) method.'),
            validateUser: Joi.func()
                .description(
                    'function to check the user credentials. async (decodedToken: token, request: Hapi.Request, h: responseToolkit) => {isValid: true|false}'
                ),
...
class HttpEndpoint {
    ...
    async setupAuthStrategies() {
        ...
        if (this.securityConfig.strategies.jwt) {
            ...
            const cfg = this.securityConfig.strategies.jwt;

            const validate = cfg.validateUser || (async () => { return { isValid: true }; });
            ...
            if (cfg.jwks) {
                this.service.log.debug(`Configure JWKS token key verification: ${JSON.stringify(cfg)}`);
                this.server.auth.strategy('jwt', 'jwt', {
                    ...
                    key: jwksRsa.hapiJwt2KeyAsync({
                        cache: cfg.jwks.cache,
                        rateLimit: cfg.jwks.rateLimit,
                        jwksRequestsPerMinute: cfg.jwks.jwksRequestsPerMinute,
                        jwksUri: cfg.jwks.jwksUri
                    }),
                    validate,
                    verifyOptions,
                });
            } else {
                this.service.log.info(`Configure static token key verification: ${JSON.stringify(cfg)}`);
    ...
    endpoints: {
        ...
        signedlink: ['/signedlink-secured.*'],
            ...
            if (
                ...
                const error = new Error(
                    'Invalid configuration style "security.endpoints: [<RegExp>,..]" configuration. When confiuring the endpoints as array "jwt" is the only supported strategy.'
                );
                this.log.error(error);
                throw error;
            }
            if (Array.isArray(this.securityConfig.endpoints) && this.securityConfig.strategies.jwt) {
                const endpointPattern = this.securityConfig.endpoints.map(routePattern => new RegExp(routePattern));
                ...
                ['jwt', 'signedlink', 'basic'].forEach(strategy => {
                    if (this.securityConfig.endpoints[strategy]) {
                        const endpointPattern = this.securityConfig.endpoints[strategy].map(
                            routePattern => new RegExp(routePattern)
                        );
                        if (this.isSecured && endpointPattern.some(endpointRegExp => path.match(endpointRegExp))) {
                            if (config.auth && config.auth.strategy !== strategy) {
                                throw new Error(
                                    `CONFIGURATION ERROR: Route: "${path}" cannot be secured using "${strategy}" strategy. It is already secured with: "${config.auth.strategy
                                    }" strategy.`);
                            }
        ...
        if (this.config.validation) {
            ...
            if (this.config.validation.verboseEndpoints) {
                const endpointPattern = this.config.validation.verboseEndpoints.map(
                    routePattern => new RegExp(routePattern)
                );
                if (endpointPattern.some(endpointRegExp => path.match(endpointRegExp))) {
            ...
            if (this.config.validation.strictEndpoints) {
                const endpointPattern = this.config.validation.strictEndpoints.map(
                    routePattern => new RegExp(routePattern)
                );
                if (endpointPattern.some(endpointRegExp => path.match(endpointRegExp))) {
