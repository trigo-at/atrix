...
describe('load-plugin', () => {
	...
	it('tries to load from package named "@trigo/atrix-plugin"', async () => {
		expect(() => loadPlugin(atrix, 'plugin')).to.throw(
			"Cannot find module '@trigo/atrix-plugin'"
		);
	});
	...
	it('validates plugin to contain "name" property', async () => {
		...
		expect(() => loadPlugin(atrix, 'test-plugin-no-name')).to.throw(
	...
	it('validates plugin to contain "version" property', async () => {
		...
		expect(() => loadPlugin(atrix, 'test-plugin-no-version')).to.throw(
	...
	it('validates plugin to contain "register" function', async () => {
		...
		expect(() => loadPlugin(atrix, 'test-plugin-no-register')).to.throw(
	...
	it('can resolve plugin from "atrix.config.pluginMap"', () => {
		atrix.config.pluginMap['test-plugin2'] = path.join(
			__dirname,
			'../tests/atrix-test-plugin2'
		);
